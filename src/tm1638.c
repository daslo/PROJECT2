...
enum SOURCE tm_source=source_d;
...
void tm_write(){
	 ...
	 *
	 */
	uint16_t n=temp[tm_source][tm_unit];
	tm_seg[5]=SYM_BLANK;
	tm_seg[4]=SYM_BLANK;

	tm_seg[3]=DIGITS[n%10];
	tm_seg[2]=DIGITS[(n%100)/10];
	tm_seg[1]=DIGITS[(n%1000)/100];
...
uint8_t tm_read(){
	...
	for(int i=0; i<1000; ++i); //wait (TODO)
...
void tm_read_buttons(){
	/*
	 * Read buttons and set display's temperature source and unit
	 *
	 * TM1638 board is very cheap hardware and can easily break
	 * if some of the buttons don't work, you can toggle through values
	 * by pressing one button :D
	 *
	 */
